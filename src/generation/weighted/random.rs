...
use crate::{symbol::SymbolBitSet, prelude::*};
...
pub trait Random {
    ...
    fn with_thread_rng<F: Fn(Symbol) -> Option<char>>(&self, start: Symbol, limit: Option<u64>, negative_rules: &[NegativeRule], to_char: F) -> Result<Vec<Symbol>, RandomGenError> {
...
impl Random for BinarizedCfg {
    fn random<R: GenRange + Clone, F: Fn(Symbol) -> Option<char>>(
        ...
        let mut backtracking: BTreeMap<usize, Vec<(&str, R, u64, usize, Vec<Symbol>)>> = BTreeMap::new();
        // let mut 
        while let Some(sym) = work.pop() {
            if terminal_set.has_sym(sym) {
                ...
                backtracking.entry(string.len() + forbidden.len()).or_insert(vec![]).push((forbidden, rng.clone(), 0, result.len(), work.clone()));
            } else {
