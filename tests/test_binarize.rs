...
fn test_binarize() {
...

    {
        ...
        let (a, b, c, x, y, g0) = equivalent.sym();
...

    {
        let mut equivalent = BinarizedCfg::new();
        let start = equivalent.start_sym();
        let (_, _, c, x, y, g0) = equivalent.sym();
    ...
    };
    {
        ...
        let (a, b) = equivalent_nulling.sym();
...
fn test_binarize_very_long_rule() {
    ...
    let mut long_rhs = cfg.terminals().take(100).collect::<Vec<_>>();
    ...
    let mut long_rhs = equivalent.terminals().take(100).collect::<Vec<_>>();
    long_rhs = long_rhs.iter().cloned().cycle().take(RULE_COUNT).collect();
