...
pub fn weighted_grammar() -> WeightedGrammar<u32> {
    ...
    let (sum, product, factor, number, plus, minus, mul, div, lparen, rparen) = bnf.sym();
    let weighted_hist = |weight, len| WeightedHistory::with_history_and_weight(History::new(0, len), weight);
    bnf.rule(sum).history(weighted_hist(1, 3)).rhs([sum, plus, product])
    .history(weighted_hist(1, 3)).rhs([sum, minus, product])
    .history(weighted_hist(3, 1)).rhs([product])
       .rule(product).history(weighted_hist(1, 3)).rhs([product, mul, factor])
       .history(weighted_hist(1, 3)).rhs([product, div, factor])
       .history(weighted_hist(3, 1)).rhs([factor])
       .rule(factor).history(weighted_hist(1, 3)).rhs([lparen, sum, rparen])
       .history(weighted_hist(3, 1)).rhs([number]);
    for _ in 0..10 {
        let sym = bnf.sym();
        bnf.rule(number).rhs(&[sym, number])
                        .rhs(&[sym]);
